...
window.addEventListener(event = "keyup", function(e){
  ...
  socket.emit('chat message', [player1.xcoord, player1.ycoord]);
  return false;
})
socket.emit('clientEvent', 'Sent an event from the client!');
  // socket.emit('getuser', 'user1')
  //now my html and js files can send stuff back and forth to each other
  //this is a big deal cuz now my server can assign player that join a user id
socket.on('backwards', function(user){
...
let otherPlayer = [[600, 300]]

//socket.on('accessGameJS', function(message){
//   console.log(message);
// })

socket.on('chat message', function(msg){
  otherPlayer = [];
  otherPlayer.push(msg);
  //logs current x-y coords
  console.log(otherPlayer[0])
...
class Component {
  constructor(name, xcoord, ycoord, color, boundaries){
    this.name = name;
    this.xcoord = xcoord;
    this.ycoord = ycoord;
...
//this is the check collision code. The player doesn't need this.
//_______________________________________________________
...
let player1 = new Player(ctx, playerX, playerY, 90, boundaries, 'red', 'player1');
...
let player2 = new Player(ctx, player2X, player2Y, 90, boundaries2, 'blue', 'player2');
//all new players get added to this list
function makePlayer3(otherPlayer){
  if (otherPlayer.length >0){
    let player3 = new Player(ctx, otherPlayer[0][0] + 100, otherPlayer[0][1] + 100, boundaries, 'black', 'player3');
    return (player3)
  }
}
let player3 = makePlayer3(otherPlayer);
...
function renderMove(xy, lr, playerList, player, otherPlayer){
  // let player3 = makePlayer3(otherPlayer);
  // playerList.push(player3)
  let coords = player.moveDirection(xy, lr);
  document.getElementById('placeholder').innerHTML = coords;
  for (let i = 0; i<playerList.length; i++){playerList[i].drawPlayer()};
  let raf = window.requestAnimationFrame(function(){renderMove(xy, lr, playerList, player, otherPlayer)});
...
window.addEventListener(event="keydown", function(e){
  ...
  switch (e.keyCode){
    ...
    case 39:
      window.requestAnimationFrame(function(){renderMove(true, 1, playerList, player1, otherPlayer)});
      console.log('right key pressed')
      console.log(otherPlayer[1])
      player3.moveTo(1000, otherPlayer[1]);
      player3.drawPlayer();
      break;
    case 37:
      window.requestAnimationFrame(function(){renderMove(true, -1, playerList, player1, otherPlayer)});
      break;
    case 38:
      window.requestAnimationFrame(function(){renderMove(false, -1, playerList, player1, otherPlayer)});
      break;
    case 40:
      window.requestAnimationFrame(function(){renderMove(false, 1, playerList, player1, otherPlayer)});
      break;
